...
class GroundRobotEnv(core.Env):
    ...
    def __init__(self, render=False, dt=0.01):
        self.viewer = None
        self._limUpPos = np.array([self.MAX_POS_BASE, self.MAX_POS_BASE, self.MAX_POS_BASE_THETA])
        self._limUpVel = np.array([self.MAX_VEL_BASE, self.MAX_VEL_BASE, self.MAX_VEL_BASE_THETA])
        self._limUpRelVel = np.array([self.MAX_VEL_FORWARD, self.MAX_VEL_BASE_THETA])
        self._limUpAcc = np.array([self.MAX_ACC_BASE, self.MAX_ACC_BASE, self.MAX_ACC_BASE_THETA])
    ...
    def integrate(self):
        # state = [x, y, theta, vel_forward, vel_turning]
        x0 = self.state[0:3]
